...
void semanticWarning(const char *message) {
    printf("AVERTISSEMENT SEMANTIQUE : %s\n", message);
    warningCount++;
}
...
void emettreString(const char* inst, const char* arg1, const char* arg2) {
    if (fd == NULL) return;
    fprintf(fd, "(%d) %s %s %s\n", instructionCounter++, inst, arg1, arg2);
}
...
void L_ID() { 
    if (symbole.ul == id) {
        int symbolIndex = findSymbol(tab_iden[symbole.att]);
        if (symbolIndex != -1) {
        ...
        declaredIds[idCount] = symbole.att;
        idCount++;
...
void TYPE() {
    ...
    for (int i = 0; i < idCount; i++) {
        addSymbol(tab_iden[declaredIds[i]], dataType);
    }
...
void I() {
    if (symbole.ul == id) {
        ...
        if (condType != integer) {
            semanticWarning("La condition devrait être de type entier");
        }
        ...
        int isReadLn = (symbole.ul == readln);
        accepter(symbole.ul);
        accepter(po);
        ...
        if (symbole.ul == id) {
            ...
            markVariableInitialized(varName);
            accepter(id);
            emettre(isReadLn ? "LireLn" : "Lire", NULL);
...
int main() {
    fp = fopen("teste_pascal.txt", "r");
    if (!fp) { 
        printf("Erreur: impossible d'ouvrir teste_pascal.txt\n"); 
        return 1; 
    }
    ...
    printf("  Instructions generees: %d\n", instructionCounter - 1);
